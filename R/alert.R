    ...
    err_validation <-
      append(
        ...
        paste(
          ...
          "]",
          ": Found [ class =",
          paste(vapply(ars[which(!is_alert_rule)], class, character(1)),
            collapse = ", "
          ),
          "]"
        )
...
alert_difference_above_perc <- function(current_period,
  ...
  function_call <- substitute(
    mean(rev(value)[cp], na.rm = TRUE) > (1 + rv / 100) * mean(rev(value)[pp], na.rm = TRUE),
