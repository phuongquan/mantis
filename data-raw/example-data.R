...
example_prescription_numbers <-
  data.frame(
    ...
    group = "SITE1",
    item_Coamoxiclav = c(
      rpois(n = 300, lambda = 50),
      rpois(n = 65, lambda = 50) +
        cumsum(floor(rnorm(n = 65, mean = 2, sd = 2)))
    ),
    item_Gentamicin = rpois(n = 365, lambda = 35),
    item_Ceftriaxone = c(
      rpois(n = 100, lambda = 30),
      rpois(n = 265, lambda = 10)
    ),
    item_Metronidazole = rpois(n = 365, lambda = 20),
    item_Meropenem = c(rpois(n = 365, lambda = 9)),
    item_Vancomycin = c(
      rpois(n = 165, lambda = 0.5),
      rep(NA, 100),
      rpois(n = 100, lambda = 0.5)
    ),
